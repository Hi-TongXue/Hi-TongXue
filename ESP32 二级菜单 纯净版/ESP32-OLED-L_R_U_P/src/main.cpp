/*==========================================================================
  ==此代码方案由 hi同学_cs制作 哔哩哔哩ID：620162563        二级菜单  纯净版===
  ==========================================================================
  //库
    U8G2                 U8g2lib.h
    按键消抖              ClickButton.h
  ...
  //代码及使用说明:
    ...
    2 lastTime  lastTime1  2  3  4 已用  
    3 GPIO已用 2  15  35  25  33  14  27  26  13
    4 蜂鸣器默认启用
    5 
...
///////////////////////////数字量  选择和菜单-数量及参数
  ...
  int xuni_key1 = 0;           //未使用 //虚拟按键：代码只循环一次
  int xuni_key2 = 0;           //未使用 //虚拟按键：代码只循环一次
...
void OLEDkaiji(){   //开机页    可以打印一些串口数据用作调试
  ...
  //u8g2.clearBuffer();
  //u8g2.drawBitmap(32, 0, 8, 64, gImage_CatHead);  //可以加开机图片
  //u8g2.sendBuffer();
  //delay(1800);
}
...
void OLEDcaidan() {       //菜单界面
  ...
  //滚动文字///////芯片型号/////////滚动文字
    ...
    if (millis() - lastTime > gundong_TXT) {
        lastTime = millis();
  ...
  ///////////////////////////////闪烁数值
    ///////////////////////////////设置
      ...
      if(data11 == 0 && data21 == 0 && data10 == 1){
        ...
        if (millis() - lastTime1 > shezhi_TXT) {
          lastTime1 = millis();
...
void xuan (){//选择按键
  ...
  //////////////////////////////////按键数值映射
    ...
    if(data3[1][2] == 1 && digitalRead(Buzzer_IO) == HIGH){
      if (millis() - lastTime4 > fengming_TXT) { // 
...
void setup() {
  ...
  //fastTimer1.setInterval(300); // 未使用  //多任务 //10MS
