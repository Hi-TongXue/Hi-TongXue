/*==========================================================================
  ...
  //BUG及已解决和代码更改记录():
    1 map线性转换
    2 模式4切换到模式6时，RGB亮度被设置为0，导致模式6彩虹渐变无法显示颜色，已修复
    3 留下调试的动画代码，方便后续测试和调整
    4 模式2&3&5 不再使用Blinker的滑块功能，改为固定数值，因为出现卡顿和偶发失灵现象
    5 NUM_LEDS和NUM_LEDS2的区别：NUM_LEDS是FastLED库中定义的LED灯珠总数量，NUM_LEDS2是控制LED灯珠数量的变量，方便后续调整和测试
