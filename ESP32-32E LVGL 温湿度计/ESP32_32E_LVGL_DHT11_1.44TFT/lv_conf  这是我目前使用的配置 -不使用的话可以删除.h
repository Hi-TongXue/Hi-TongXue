...
/** Default display refresh, input device read and animation step period. *//** 默认显示刷新、输入设备读取和动画步长周期。*/
//#define LV_DEF_REFR_PERIOD  33      /**< [ms] 30FPS */
#define LV_DEF_REFR_PERIOD  16      /**< [ms] 60FPS */
//#define LV_DEF_REFR_PERIOD  10      /**< [ms] 100FPS */
//#define LV_DEF_REFR_PERIOD  1      /**< [ms] FPS */

/** Default Dots Per Inch. Used to initialize default sizes such as widgets sized, style paddings.
 * (Not so important, you can adjust it to modify default sizes and spaces.) */

/** 每英寸默认点数。用于初始化默认尺寸，例如小部件大小、样式间距等。
 *（不那么重要，你可以调整它来修改默认的尺寸和间距。）*/
...
#define LV_USE_LOG 1
     ...
     *  - LV_LOG_LEVEL_WARN     当发生意外情况但未导致问题时进行记录。  
     *  - LV_LOG_LEVEL_ERROR    仅记录可能引发系统故障的严重问题。  
     *  - LV_LOG_LEVEL_USER     仅记录用户自定义的日志消息。  
...
/** 使用内置的对象ID处理函数：  
* - lv_obj_assign_id：当创建一个控件时调用。为每个控件类使用独立计数器作为ID。  
* - lv_obj_id_compare：比较ID，以判断其是否与请求的值匹配。  
* - lv_obj_stringify_id：返回字符串化的标识符，例如“button3”。  
* - lv_obj_free_id：不执行任何操作，因为ID无需分配内存。  
当禁用这些功能时，用户需自行实现上述函数。*/
...
/* 使用 ASCII 范围和部分符号的 Montserrat 字体，bpp = 4 
 * https://fonts.google.com/specimen/Montserrat */
...
#define LV_FONT_DEJAVU_16_PERSIAN_HEBREW    1  /**< Hebrew, Arabic, Persian letters and all their forms *//**< 希伯来文、阿拉伯文、波斯文字母及其所有形式 */
#define LV_FONT_SIMSUN_14_CJK               1  /**< 1000 most common CJK radicals *//**< 1000个最常见的汉字部首 */
#define LV_FONT_SIMSUN_16_CJK               1  /**< 1000 most common CJK radicals *//**< 1000个最常见的汉字部首 */
#define LV_FONT_SOURCE_HAN_SANS_SC_14_CJK   1  /**< 1338 most common CJK radicals *//**< 1338个最常见的汉字部首 */
#define LV_FONT_SOURCE_HAN_SANS_SC_16_CJK   1  /**< 1338 most common CJK radicals *//**< 1338个最常见的汉字部首 */

/** Pixel perfect monospaced fonts *//** 完美像素间距的等宽字体 */
...
/** 始终设置默认字体 */
//#define LV_FONT_DEFAULT &lv_font_montserrat_14        //默认
...
/** 启用对大字体和/或包含大量字符的字体的处理。  
 * 限制取决于字体大小、字体类型和每像素点数（bpp）。  
 * 如果字体需要此功能，将触发编译器错误。
 */
 ...
 * 小部件  
 *================*/ 
/* 小部件的文档可在此处查看：https://docs.lvgl.io/master/details/widgets/index.html 。 */  

/** 1: 在创建时为这些小部件设置默认值。  
 * - lv_buttonmatrix_t：获取默认映射：{"Btn1", "Btn2", "Btn3", "\n", "Btn4", "Btn5", ""}，否则不设置映射。  
 * - lv_checkbox_t：字符串标签设为"Check box"，否则设为空字符串。  
 * - lv_dropdown_t：选项设为"Option 1"、"Option 2"、"Option 3"，否则不设置任何值。  
 * - lv_roller_t：选项设为"Option 1"、"Option 2"、"Option 3"、"Option 4"、"Option 5"，否则不设置任何值。  
 * - lv_label_t：文本设为"Text"，否则设为空字符串。  
 * */
...
#if LV_USE_SPAN
    /** A line of text can contain this maximum number of span descriptors. *//**一行文本最多可以包含此数量的跨度描述符。*/
...
#define LV_USE_SYSMON   0
    ...
    #define LV_USE_PERF_MONITOR 1
        ...
        //#define LV_USE_PERF_MONITOR_POS LV_ALIGN_BOTTOM_RIGHT     //显示位置 默认右下角
        #define LV_USE_PERF_MONITOR_POS LV_ALIGN_TOP_RIGHT      //显示位置 右上角
    ...
    #define LV_USE_MEM_MONITOR 1
        ...
        #define LV_USE_MEM_MONITOR_POS LV_ALIGN_BOTTOM_LEFT     //显示位置 默认左下角
    #endif
#endif /*LV_USE_SYSMON*/
